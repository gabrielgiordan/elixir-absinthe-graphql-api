defmodule BlogApi.Blog do
  ...
  def list_posts(filter) do
    filter
    |> Enum.reduce(Post, fn {:filter, filter}, entity ->
      filter_with(entity, filter)
    end)
    |> Repo.all
  end
  ...
  def filter_with(entity, filter) do
    ...
    |> Enum.reduce(entity, fn
      {:site, site}, entity -> from e in entity, where:
        fragment("?->>? ILIKE ?", e.details, "site", ^site)
      {:section, section}, entity -> from e in entity, where:
        fragment("?->>? ILIKE ?", e.details, "section", ^section)
      {:topic, topic}, entity -> from e in entity, where:
